...
Description: Implementation of PSO CNN compression 
...
def logger_fc(pso_cont:PSOController, before_acc:float, save_data:pd.DataFrame=None) -> pd.DataFrame:
    ...
    # saving progress
    save_data = save_data.append(pd.DataFrame(new_data).astype(CompressConfig.PSO_DATA_TYPES))
    if pso_cont.time % CompressConfig.SAVE_EVERY == CompressConfig.SAVE_EVERY - 1:
        save_data.reset_index(drop=True, inplace=True)
        save_data.to_csv(CompressConfig.PSO_SAVE_FILE, index=False)

    return save_data

def compression_pso_optim(num_iterations:int, num_particles:int, ranges:list, before_loss:float, fit_controller:FitnessController) -> list:
    """PSO compression impelentation.

    Args:
        num_iterations (int): Number of iterations for the PSO optimization.
        num_particles (int): Number of particles in the PSO search.
